...
ASSETS_PATH = OUTPUT_PATH / Path(r"D:\Documentos\ESCOM\VISION ARTIFICIAL\Rec_manos_lenguaje\ASL\build\assets\frame0")
...
def mostrar_toast(mensaje, x, y, borrarAutomatico: bool = True):
    toast = tk.Toplevel()
...
def draw_region(image, center):
    cropped_image = cv2.rectangle(image, (center[0] - 130, center[1] - 130),
        (center[0] + 130, center[1] + 130), (184, 0, 144), 2)
...
def AddCharToWord(word, curr_char):
    temp_word = word
    if curr_char == 'space':
        mostrar_toast("Palabra agregada" , "750", "450")
        temp_word = ""
    elif curr_char == 'del':
...
"""VIDEO"""
def frame_video_stream(names, curr_char, prev_char, word, *args):
    global sentence
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            try:
                ...
                char = cv2.putText(full_img, curr_char, (center[0]-135, center[1]-135), cv2.FONT_HERSHEY_SIMPLEX, 1, (184, 0, 144), 2, cv2.LINE_AA)
                char_prob = cv2.putText(full_img, '{0:.2f}'.format(np.max(pred)), (center[0]+60, center[1]-135), cv2.FONT_HERSHEY_SIMPLEX, 1, (184, 0, 144), 2, cv2.LINE_AA)
...
window.geometry("1138x720+200+0")
...
def copiar_texto():
    ...
    mostrar_toast("Texto copiado en el portapapeles", "370", "340")
